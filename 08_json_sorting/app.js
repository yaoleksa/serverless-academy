const http = require('http');

const endpoints = `https://jsonbase.com/sls-team/json-793
https://jsonbase.com/sls-team/json-955
https://jsonbase.com/sls-team/json-231
https://jsonbase.com/sls-team/json-931
https://jsonbase.com/sls-team/json-93
https://jsonbase.com/sls-team/json-342
https://jsonbase.com/sls-team/json-770
https://jsonbase.com/sls-team/json-491
https://jsonbase.com/sls-team/json-281
https://jsonbase.com/sls-team/json-718
https://jsonbase.com/sls-team/json-310
https://jsonbase.com/sls-team/json-806
https://jsonbase.com/sls-team/json-469
https://jsonbase.com/sls-team/json-258
https://jsonbase.com/sls-team/json-516
https://jsonbase.com/sls-team/json-79
https://jsonbase.com/sls-team/json-706
https://jsonbase.com/sls-team/json-521
https://jsonbase.com/sls-team/json-350
https://jsonbase.com/sls-team/json-64`.split('\n').forEach(endpoint => {
    makeCall(endpoint, 0);
});

let counter = 0;
let success = 0;

function makeCall(url, tryNum) {
    if(tryNum >= 3) {
        console.log(`[Fail] ${url}: The endpoint is unavailable`);
        return;
    }
    try {
        http.get(url, res => {
            const data = [];
            res.on('data', chunk => {
                data.push(chunk);
            });
            res.on('end', () => {
                if(findByKey(JSON.parse(Buffer.concat(chunk).toString()), "isDone")) {
                    console.log(`[Success] ${url}: isDone - True`);
                    success++;
                } else {
                    console.log(`[Success] ${url}: isDone - False`);
                }
                if(counter == 20) {
                    console.log(`Found True Values: ${success}`);
                    console.log(`Found False Values: ${20 - success}`);
                }
                counter++;
            });
        });
    } catch(exception) {
        tryNum++;
        return makeCall(url, tryNum);
    }
}

function findByKey(obj, key) {
    for(let i in obj) {
        if(i == key) {
            return obj[key];
        }
        if(typeof obj[i] == 'object' && findByKey(obj[i], key)) {
            return findByKey(obj[i], key);
        }
    }
}